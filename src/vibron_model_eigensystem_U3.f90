PROGRAM vibron_eigensystem_u3
  ...
  IF (L_min /= 0 .AND. excitation) THEN
     ...
     ENDIF
     !
     CALL U3_BASIS_VIBRON(N_val, 0_I4B) ! Build U3 basis
     !
     !
     ...
     !
     ! BUILD HAMILTONIAN
     CALL U3_HAMILTONIAN_VIBRON(N_val, 0_I4B, dim_block)
     !      
     ! Check time
     CALL CPU_TIME(time_check)
  ...
  ang_momentum : DO L_val = L_min, L_max
     ...
     IF (save_ham) &
          CALL SAVE_HAM_MATRIX(N_val, L_val, dim_block, "u3", Ham_U4_mat)
     !      
     ! Check time
     CALL CPU_TIME(time_check)
     ...
     DO U3_state = 1, dim_block
        ...
        !
        WRITE(UNIT = *, FMT = *) np, eigenval_vec(U3_state)
        !
     ENDDO
     !
     ! Save eigenvector components
